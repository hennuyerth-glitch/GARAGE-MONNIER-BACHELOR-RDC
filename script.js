...
.reservation-modal{width:min(820px,100%);max-height:94vh;overflow:auto;background:#fff;border-radius:10px;box-shadow:0 25px 80px rgba(0,0,0,.28);color:#122235}
...
.reservation-field input,.reservation-field select,.reservation-field textarea{width:100%;box-sizing:border-box;border:1px solid #cbd6e0;border-radius:5px;padding:11px 12px;font:inherit;color:#122235;background:#fff}.reservation-field textarea{min-height:90px;resize:vertical}
...
.vehicle-extra{display:none;grid-column:1/-1;background:#f4f7fa;border:1px solid #dce4eb;border-radius:7px;padding:15px}.vehicle-extra.is-visible{display:grid;grid-template-columns:1fr 1fr;gap:15px}
.time-picker{grid-column:1/-1;background:#f4f7fa;border:1px solid #dce4eb;border-radius:7px;padding:15px}.time-picker-label{font-size:11px;font-weight:800;text-transform:uppercase;letter-spacing:.06em;color:#33465a;margin-bottom:10px}
.time-scroller{display:flex;gap:8px;overflow-x:auto;padding:4px 2px 9px;scroll-snap-type:x proximity;-webkit-overflow-scrolling:touch}.time-scroller::-webkit-scrollbar{height:6px}.time-scroller::-webkit-scrollbar-thumb{background:#b7c6d4;border-radius:10px}
.time-option{flex:0 0 82px;scroll-snap-align:start;border:1px solid #cbd6e0;background:#fff;color:#122235;border-radius:6px;padding:11px 8px;text-align:center;font-weight:800;cursor:pointer;transition:transform .18s ease,background .18s ease,border-color .18s ease,box-shadow .18s ease}.time-option:hover{transform:translateY(-2px);border-color:#003b70}.time-option.selected{background:#003b70;color:#fff;border-color:#003b70;box-shadow:0 5px 14px rgba(0,59,112,.2)}
...
if (nav && !document.querySelector('.nav-reservation')) {
  ...
  link.className = 'nav-reservation'; link.href = '#reservation'; link.textContent = 'Réserver';
...
if (!document.getElementById('reservationOverlay')) {
  ...
  overlay.id = 'reservationOverlay'; overlay.className = 'reservation-overlay';
  overlay.innerHTML = `
    <div class="reservation-modal" role="dialog" aria-modal="true" aria-labelledby="reservationTitle">
      <div class="reservation-head">
        <div><div style="font-size:11px;font-weight:800;letter-spacing:.16em;color:#f4c400">PRISE DE RENDEZ-VOUS</div><h2 id="reservationTitle">Réserver au Garage Monnier</h2><p>Votre demande est envoyée directement au garage. Le rendez-vous reste soumis à confirmation.</p></div>
        <button class="reservation-close" type="button" aria-label="Fermer">×</button>
      </div>
      <div class="reservation-body">
        <form id="reservationForm" action="https://formsubmit.co/monnier.garageeglats@orange.fr" method="POST">
          <input type="hidden" name="_subject" value="Nouvelle demande de rendez-vous – Garage Monnier">
          <input type="hidden" name="_template" value="table">
          <input type="hidden" name="_captcha" value="true">
          <input type="hidden" name="_next" value="https://hennuyerth-glitch.github.io/GARAGE-MONNIER-BACHELOR-RDC/merci-reservation.html">
          <input type="hidden" name="_url" value="https://hennuyerth-glitch.github.io/GARAGE-MONNIER-BACHELOR-RDC/">
          <div class="reservation-grid">
            <div class="reservation-field full"><label for="reservationService">Prestation</label><select id="reservationService" name="prestation" required><option value="">Choisir une prestation</option><option>Entretien / révision</option><option>Réparation / diagnostic</option><option>Véhicule électrique ou hybride</option><option>Pare-brise</option><option>Location courte durée</option><option>Vente de véhicule</option><option>Autre demande</option></select></div>
            <div class="reservation-field"><label for="reservationVehicleType">Type de véhicule</label><select id="reservationVehicleType" name="type_vehicule" required><option value="">Choisir</option><option>Citadine</option><option>Berline</option><option>SUV / crossover</option><option>Utilitaire</option><option>Minibus</option><option>Moto</option><option>Autre</option></select></div>
            <div class="reservation-field"><label for="reservationEnergy">Motorisation</label><select id="reservationEnergy" name="motorisation" required><option value="">Choisir</option><option>Essence</option><option>Diesel</option><option>Hybride</option><option>100 % électrique</option><option>Autre</option></select></div>
            <div class="vehicle-extra" id="vehicleExtra"><div class="reservation-field"><label for="reservationBrand">Marque / modèle</label><input id="reservationBrand" name="marque_modele" type="text" placeholder="Ex. Renault Clio V"></div><div class="reservation-field"><label for="reservationPlate">Immatriculation</label><input id="reservationPlate" name="immatriculation" type="text" placeholder="AA-123-AA"></div></div>
            <div class="reservation-field"><label for="reservationDate">Date souhaitée</label><input id="reservationDate" name="date_souhaitee" type="date" required></div>
            <div class="time-picker"><div class="time-picker-label">Choisissez une heure précise</div><input type="hidden" id="reservationTime" name="heure_souhaitee" required><div class="time-scroller" id="timeScroller" aria-label="Choix de l'heure"></div><div class="time-hint">Faites défiler horizontalement pour voir tous les créneaux. Les horaires proposés sont indicatifs et doivent être confirmés par l'équipe.</div></div>
            <div class="reservation-field"><label for="reservationName">Nom & prénom</label><input id="reservationName" name="nom_prenom" type="text" autocomplete="name" placeholder="Votre nom" required></div>
            <div class="reservation-field"><label for="reservationPhone">Téléphone</label><input id="reservationPhone" name="telephone" type="tel" autocomplete="tel" placeholder="06 00 00 00 00" required></div>
            <div class="reservation-field full"><label for="reservationEmail">E-mail</label><input id="reservationEmail" name="email" type="email" autocomplete="email" placeholder="vous@exemple.fr" required></div>
            <div class="reservation-field full"><label for="reservationMessage">Précisions</label><textarea id="reservationMessage" name="message" placeholder="Décrivez votre besoin, le voyant allumé, vos contraintes ou toute information utile…"></textarea></div>
          </div>
          <label class="reservation-consent"><input type="checkbox" name="consentement" value="Oui" required><span>J'accepte que les informations saisies soient transmises au Garage Monnier uniquement pour traiter ma demande de rendez-vous.</span></label>
          <div class="reservation-actions"><button class="reservation-btn reservation-btn-primary" type="submit">Envoyer ma demande au garage</button><button class="reservation-btn reservation-btn-secondary" type="button" id="reservationCall">Appeler le garage</button></div>
          <div class="reservation-note"><strong>Après l'envoi :</strong> le garage reçoit votre demande par e-mail. Vous êtes invité à vérifier votre boîte mail et le garage vous confirme ensuite la disponibilité du créneau. Pour une panne ou une urgence, appelez le <strong>06 13 27 19 85</strong>.</div>
        </form>
      </div>
    </div>`;
  ...
  times.forEach(time => {
    const button = document.createElement('button'); button.type='button'; button.className='time-option'; button.textContent=time; button.setAttribute('aria-label',`Choisir ${time}`);
    button.addEventListener('click',()=>{
      document.querySelectorAll('.time-option').forEach(b=>b.classList.remove('selected'));
      button.classList.add('selected'); timeInput.value=time;
      button.scrollIntoView({behavior:'smooth',inline:'center',block:'nearest'});
    });
    timeScroller.appendChild(button);
  });

  document.getElementById('reservationVehicleType').addEventListener('change',()=>document.getElementById('vehicleExtra').classList.add('is-visible'));
  document.getElementById('reservationEnergy').addEventListener('change',()=>document.getElementById('vehicleExtra').classList.add('is-visible'));
  document.querySelector('.reservation-close').addEventListener('click', closeReservation);
